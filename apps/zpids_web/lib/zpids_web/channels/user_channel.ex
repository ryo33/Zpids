defmodule Zpids.Web.UserChannel do
  ...
  def init(socket, _user_id) do
    client_id = Zpids.ID.gen()
    listen(Display.Event.for(client_id))
    Display.start_link(client_id)
    game_module = Application.get_env(:zpids_web, Zpids.Web)[:zpids_game_module]
    client_module = Application.get_env(:zpids_web, Zpids.Web)[:zpids_game_client_module]
